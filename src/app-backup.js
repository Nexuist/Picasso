...
/*
	...
	methods:
		...
		openExternal() - Opens the fileURL in the operating system's h andler of choice
...
let root = new Vue({
	...
	data: {
		...
		screen: "upload",
		supportedFileTypes: ["png", "jpg", "jpeg", "gif", "mp4", "webm"],
	...
	created: function() {
		document.title = "Picasso v" + this.version; // Need to use 'this' here because 'root' isn't available yet
		bus.$on("clearModal", () => {
			...
			.then((media) => {
				if (media.length == 0) {
					...
					helper.getSettingsForFolder(folder)
						...
						.catch((err) => {
							if (err.code === "ENOENT") return; // Don't error if there's no settings to be retrieved
